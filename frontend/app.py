...
st.markdown("""
<style>
/* warm professional -- dark with teal accents */
...
with st.sidebar:
    ...
    if privacy_mode:
        st.info("Your query stays on our servers only. Never sent to third-party AI.")
    ...
    st.divider()
    st.markdown("**Debug**")
    debug = st.checkbox("Show raw JSON response")
...
if run and query.strip():
    with st.spinner("Running agents -- fetching from all sources..."):
        try:
            ...
            # sources used
            st.markdown("**Sources queried:** " + " ".join([f'<span class="source-tag">{s}</span>' for s in r.get("sources_used", [])]), unsafe_allow_html=True)

        except httpx.ConnectError:
            st.error("Cannot connect to API. Make sure uvicorn is running in Terminal 1.")
