...
def ingest(papers: List[Dict], source: str = "unknown"):
    col = get_col()
    docs, embs, metas, ids = [], [], [], []
    seen_ids = set()  # dedup within this batch before upsert
    for p in papers:
        text = f"{p.get('title', '')} {p.get('abstract', '')}"
        for i, c in enumerate(chunk(text)):
            # add index to hash so two identical chunks get unique ids
            doc_id = hashlib.md5(f"{c}{i}{source}".encode()).hexdigest()
